...
import quotedecor.company_quotesTab.CompanyQuotesTabTitlePaneCtrl;
...
public class CompanyProjectsTabAccordionCtrl implements Initializable {
...
//                    System.out.println(roomsRS + "Num of Rooms!!!!!!!!!!!!!!!!!!!");
                    ...
                    if (companyIdrs.equals(companyID) && isConfirmed == true) {

                        FXMLLoader loader = new FXMLLoader(getClass().getResource(URLS.COMPANY_PROJECT_TITLE_PANE));
                        TitledPane projectTitlePane = (TitledPane) loader.load();
                        CompanyProjectTitlePaneCtrl CompanyProjectTitlePaneCtrl = loader.getController();
                        projectTitlePane.setText("Project ID: " + projectId);
                        //CompanyProjectTitlePaneCtrl.setLbl_projectID(String.valueOf(projectId));
                        CompanyProjectTitlePaneCtrl.setLbl_customerID(customerId);
                        CompanyProjectTitlePaneCtrl.setLbl_projectName(projectName);
                        CompanyProjectTitlePaneCtrl.setLbl_Address(customerAddress);
                        CompanyProjectTitlePaneCtrl.setLbl_NoOfRooms(String.valueOf(roomCount));
                        CompanyProjectTitlePaneCtrl.setLbl_Price(String.valueOf(price));
            ...
            if (accordion.getPanes().size() == 0) {
                ...
                CompanyProjectTitlePaneCtrl.removeLbl_ConfirmedProject();
                CompanyProjectTitlePaneCtrl.setLbl_NoProjectsMessage("CURRENTLY NO CONFIRMED PROJECTS....");
